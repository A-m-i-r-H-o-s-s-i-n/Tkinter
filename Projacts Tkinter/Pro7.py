...
def getWeather():
    try:
        ...
        if temp > 0:
            ...
            home= Label(font=("arial",25,"bold"))
            home.place(x=400,y=280)
            home.config(text="Good weather for walking")
            
        else:
            t.config(text=(f"{temp}°"))
            c.config(text=(condition,"|","FEELS","LIKE",temp,"∘"))
            home= Label(font=("arial",20,"bold"))
            home.place(x=400,y=280)
            home.config(text="Cold weather is not good for walking")
...
#logo
Logo_iage = PhotoImage(file="Images/Copy of logo.png")
